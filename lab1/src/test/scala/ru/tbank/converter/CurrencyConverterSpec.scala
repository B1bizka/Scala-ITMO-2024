...
class CurrencyConverterSpec extends AnyFlatSpec with Matchers {
  ...
  "exchange" should "throw UnsupportedCurrencyException if rates dictionary doesn't contain second necessary currency" in {
    val rates = Map(
      "USD" -> Map("RUB" -> BigDecimal(72.5)),
      "RUB" -> Map("USD" -> BigDecimal(1 / 72.5))
    )
    val converter = CurrencyConverter(rates)
    a[UnsupportedCurrencyException] should be thrownBy converter.exchange(Money(1, "USD"), "EUR")
  }
  it should "throw UnsupportedCurrencyException if rates dictionary doesn't contain first necessary currency" in {
    val rates = Map(
      "USD" -> Map("RUB" -> BigDecimal(72.5)),
      "RUB" -> Map("USD" -> BigDecimal(1 / 72.5))
    )
    val converter = CurrencyConverter(rates)
    a[UnsupportedCurrencyException] should be thrownBy converter.exchange(Money(1, "EUR"), "RUB")
  }
}
